...
- 「書きたい」「作りたい」「ドラフト」「雛形」を含む → **write モード** (パスが併記されていても優先。パスは保存先候補として扱う)
- 「レビュー」「チェック」「lint」を含む、または `$ARGUMENTS` にファイルパスのみ → **review モード**
- どちらか不明 → ユーザに確認する
...
**引数表記**: イベント・アクションが引数を取る場合は `name(arg1, arg2)` の関数形式で書く。

- イベント: `pick(item)`、`insert_bill(amount)` — 受信時の payload
- アクション: `publish_failed(current_phase)` — 実行時の引数
- ガードで引数参照: `pick(item) [item ∈ displayedItems]`

引数の型・形式は multi-entity ではイベント契約表の `payload` 列、単一エンティティでは設計メモに別途定義する。

**CSP/TLA+ 翻訳との接続**: 引数を関数形式で統一しておくと CSP の channel-passing (`c!x` / `c?x`) や TLA+ の `Action(x) == ...` パラメータへの移植が直接対応する (前述の動作 ID 化の延長線)。
...
- **モード依存** (例: 同じボタンがモードで挙動分岐)
  ...
  モードを階層 (composite) に切り出し、同名イベントを各モード内で別の遷移として書く。現在のモードに応じて 1 つの遷移が選ばれる (= mode dependence)。直積爆発を回避できる。

  **broadcast との混同に注意**: 同名イベントを**階層**に書けばモード依存 (排他選択)、**直交領域** (`--` 区切り) に書けば broadcast (並列発火) になる。記法は同じだが書く場所で意味が変わる。
...
未定義の状態×イベントは「無視 (self-loop / no-op)」「エラー」のどちらかを設計メモで宣言する (E01 の根本対処)。
...
- **終了**: 終了状態は `stateID --> [*]` で書く。ガード付きの終了判定は `event [guard] / action` の慣習に従い `stateID --> [*] : tick [retry_exhausted] / cleanup` の形で書く (ガードは必ず `[ ]` で囲む)。
...
- broadcast の対応: (どのイベントが何領域に broadcast されるか / 該当なしなら「なし」)
- ガードの根拠: (なぜそのガードが必要か。参照変数のスコープも)
- アクションの冪等性: (累積系/冪等系の区別、リトライ時の振る舞い)
- 未定義イベントの扱い: (無視 (self-loop / no-op) / エラー のどちらを既定にするか)
- 異常系のカバレッジ: (キャンセル / 失敗 / タイムアウト等を書き漏らしていないか)
- 既知の未対応ケース: (意図的に省いた組合せ)

**該当時**:
- イベント契約 (multi-entity): 共有イベントの producer/consumer/sync性 (詳細は別セクション「イベント契約表」)
- 共有状態の排他制御 (multi-entity): どの変数を誰が書き、どう競合を防ぐか
- refinement 子 spec のリンク: 抽象状態 → 詳細 spec ファイルへのリンク
- 実装詳細 doc のリンク (xxx-impl.md): 振る舞いと実装を別 doc に切っている場合の相互リンク
```
...
- [ ] (実装詳細を別 doc に分離している場合) `xxx-spec.md` と `xxx-impl.md` が相互リンクしている
- [ ] 設計メモがファイル末尾にあり、**必須項目** (設計メモテンプレの `必須` 節) がすべて埋まっている。該当時項目 (multi-entity / refinement / 実装詳細分離) も該当すれば埋める
...
- **E04 変換系・リアクティブ系の混入**: 履歴に依存しない入出力対応が状態機械側に混入している、またはその逆。
  ...
  - **E04 と W04 は別軸**: アクションが状態 (履歴・カウンタ・蓄積) を更新する遷移 (例: `total += X`、`count++`) は E04 を疑う対象外 (リアクティブとして正常)。W04 (階層化不足) は別軸の判定で、**同パターンの遷移が複数状態で繰り返される**ことが基準。両者を混同しない。
...
- **W06 アクションの非冪等性が未文書化**: アクション名から副作用の累積性が読み取れず、設計メモにも冪等性の記述がない (例: `update_state` という曖昧名)。リトライ時の動作が不明。
- **W07 イベント契約の同期性未定義** (multi-entity の場合): 共有イベントが複数のエンティティ・領域に流れているのに、sync (発火元が完了を待つ) か async (発火しっぱなし) かが**イベント契約表** (`references/multi-entity-composition.md` 参照) に明記されていない。race condition の原因になりうる。multi-entity では契約表自体を必須とする。
- **W08 共有状態の競合可能性** (multi-entity の場合): 複数の領域・別図が同じ変数を読み書きしているのに、排他制御方針 (lock / 単一書き込み元 / CAS 等) が設計メモにない。
- **W09 実装詳細の混入**: 振る舞い側にミドルウェア固有名詞 (SNS / SQS / Kafka / Lambda / Fargate / DynamoDB 等) や関数名・queue 名・topic ARN が書かれている。配信保証・通信形態・永続性などの**抽象的性質**に置き換え、ミドルウェア選定は別 doc (`xxx-impl.md`) に隔離する余地。
