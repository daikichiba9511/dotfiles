...
description: "Use for ambiguous or high-cost problems where a wrong framing could materially change the solution, such as unfamiliar root-cause investigations, architecture choices with broad impact, or uncertain experiment failures. Do not use for straightforward implementation, routine fixes, or tasks with an obvious verifiable path."
allowed-tools: Read, Glob, Grep, WebSearch, WebFetch, Bash(git log:*), Bash(git show:*), Bash(git diff:*)
---
