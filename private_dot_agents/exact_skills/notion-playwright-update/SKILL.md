---
name: notion-playwright-update
allowed-tools: Bash(deno:*), Read, Write, Glob
description: "Use when the user wants to replace an existing Notion page body from local Markdown through a real Playwright browser session and reset its title. Do not use for Notion API work, reading Notion, partial edits, append operations, or general browser automation."
---
...
```bash
deno run -A ~/.agents/skills/notion-playwright-update/scripts/update_notion_page.ts \
  --url "<notion page url>" \
  --md  "<path/to/body.md>" \
  --title "<page title>"
```
